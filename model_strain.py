...
class StrainLayer(nn.Module):
    ...
    def forward(self, displacement):
        ...
        sobel_x = torch.tensor([[-1, 0, 1],
                                [-2, 0, 2],
                                [-1, 0, 1]], dtype=displacement.dtype, device=displacement.device).view(1, 1, 3, 3)
        sobel_y = torch.tensor([[-1, -2, -1],
                                [0, 0, 0],
                                [1, 2, 1]], dtype=displacement.dtype, device=displacement.device).view(1, 1, 3, 3)
        # Compute gradients
        strain_xx = torch.nn.functional.conv2d(disp_x, sobel_x, padding=1)
        strain_yy = torch.nn.functional.conv2d(disp_y, sobel_y, padding=1)
        strain_xy = 0.5 * (torch.nn.functional.conv2d(disp_y, sobel_x, padding=1) +
                           torch.nn.functional.conv2d(disp_x, sobel_y, padding=1))
        return torch.cat([strain_xx, strain_yy, strain_xy], dim=1)
